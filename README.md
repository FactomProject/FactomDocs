...
NotaryChain Servers provide for simple “proof of existence” entries.  In addition, these Servers implement validation scripts that allow for chains of notarized entries, thus the name NotaryChains.  NotaryChains can be used to implement token systems, asset trading systems, smart contracts, and more.   A federated set of NotaryChains provides for real time audits, easy transfer from one NotaryChain server to another, reduced blockchain pollution, and other benefits.
